...
class PasswordColumn extends TextColumn 
{
	...
	function checkBeforeSave( &$request )
	{
		...
		if( $checkto!="" && $request[$this->myName]!=$request[$this->myEqualTo] )
		{
			$this->myError="Passwords should match";//$langdata['reg_password_mistmach'];//"Введенные пароли не совпадают<br>";
			return false;
		}
		return true;
	}
	...
	{
			$ret="<input type='password' id='$this->myName' name='$this->myName' value='".$this->getValue( $row )."'".
					" class=\"addann_input".
					(isset($this->myRequired)?" validate['required',confirm[$this->myEqualTo]]":"").
					"\" $this->myAdditional  ";
